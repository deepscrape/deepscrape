  ...
  async (request) => {
    const data = validateCallableData(
      z.object({
        ...
        metrics: z.object({
          ...
          // Passive signals hash (canvas/WebGL/hardware). A risk input, never identity.
          deviceSignals: z.string().max(512).optional().default(""),
        }),
      }),
      request.data
    )
    ...
    try {
      ...
      const sessionData = {
        ...
        location: resolvedLocation,
        deviceFingerprint,
        deviceSignalsHash: metrics.deviceSignals,
        region: resolvedGeo.region,
        country: resolvedGeo.country,
